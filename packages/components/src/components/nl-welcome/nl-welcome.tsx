...
export class NlWelcome {
  ...
  render() {
    ...
    const local = this.allowAuthMethod('local') && state.localSignup;
    const signup = local || this.allowAuthMethod('connect');
      ...
      <Fragment>
        ...
        {this.allowAuthMethod('connect') && (
          ...
          {this.hasOTP && this.allowAuthMethod('otp') && (
            <button-base titleBtn="Log in with DM" onClick={() => this.handleChangeScreen(CURRENT_MODULE.LOGIN_OTP)}>
              <svg style={{ display: 'none' }} slot="icon-start" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
          ...
          {signup && (
            <div class="nl-divider py-3 flex items-center text-xs uppercase before:flex-[1_1_0%] before:border-t before:me-6 after:flex-[1_1_0%] after:border-t  after:ms-6">
              Or
            </div>
          )}
          {signup && (
            <button-base onClick={() => this.handleChangeScreen(local ? CURRENT_MODULE.LOCAL_SIGNUP : CURRENT_MODULE.SIGNUP)} titleBtn="Sign up">
              <svg style={{ display: 'none' }} slot="icon-start" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
