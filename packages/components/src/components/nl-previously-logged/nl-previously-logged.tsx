...
export class NlPreviouslyLogged {
  ...
  @Event() nlSwitchAccount: EventEmitter<string>;
  @Event() nlLoginRecentAccount: EventEmitter<string>;
  ...
  handleRemoveRecent(pubkey: string) {
    const recents = this.recents.filter(el => el.pubkey !== pubkey);
  ...
  handleSwitch(pubkey: string) {
    this.nlSwitchAccount.emit(pubkey);
  }

  handleLoginRecentAccount(pubkey: string) {
    this.nlLoginRecentAccount.emit(pubkey);
  }
          ...
          <div class="max-w-96 mx-auto">
            ...
            <ul class="p-2 rounded-lg border border-blue-200 flex flex-col w-full gap-0.5">
              {this.accounts.map(el => {
                ...
                return (
                  <li
                    onClick={() => this.handleSwitch(el.pubkey)}
          ...
          <div class="max-w-96 mx-auto pt-5">
            ...
            <ul class="p-2 rounded-lg border border-gray-200 flex flex-col w-full gap-0.5">
              {this.recents.map(el => {
                ...
                return (
                  <li
                    onClick={() => this.handleLoginRecentAccount(el.pubkey)}
                    class="hover:bg-gray-100 flex cursor-pointer items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm justify-between"
                  >
                    <div class="flex items-center gap-x-3.5 w-full">
                      <div class="w-full max-w-7 h-7 flex relative">
                    ...
                    <div class="flex gap-2">
                      ...
                      <svg
                        onClick={e => {
                          e.stopPropagation();
                          this.handleRemoveRecent(el.pubkey);
                        }}
                        ...
                        stroke-width="1.5"
                        stroke="currentColor"
                        class="w-6 h-6 text-red-500 hover:text-red-600"
                      >
                        <path
                          stroke-linecap="round"
                          stroke-linejoin="round"
